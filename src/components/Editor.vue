...
const props = withDefaults(defineProps<EditorProps>(), example.props)
...
<template>
  <div class="iw-editor">
    ...
    <el-row class="iw-editor-formula">
      <CmWrap ref="CmWrapRef" class="iw-editor-formula--size" v-model:formulaValue="formulaValue" :targetGuard="targetVar" :materials="materials" :entrance="entrance" />
    </el-row>
              ...
              <template v-for="materialFun in materialFuns">
                <el-tab-pane :label="materialFun.nsLabel">
                  <el-tree :data="materialFun.items" node-key="id" accordion empty-text="暂无数据">
                    <template #default="{ node, data }">
                      <div class="iw-editor-material__item" @click="insertMaterial(node.isLeaf, materialFun.nsName, data.name)" @mouseenter="materialNote = data.note" @mouseleave="materialNote = ''">
                        <p class="iw-editor-material__item-tile">{{ data.name }}</p>
                        <p class="iw-editor-material__item-note">{{ data.label }}</p>
                      </div>
                    </template>
