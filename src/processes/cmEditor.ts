...
export function diagnosticFormula(currNode: SyntaxNode, state: EditorState, expectedOutputKind: iwInterface.VarKind, diagnostics: Diagnostic[], entrance: string, diagnosedNode: [number, number][], findMaterials: (ns: string) => Namespace | undefined, processHit: (name: string) => void): void {
  ...
  if (currNode.name === 'ParenthesizedExpression') {
    ...
    const expr = currNode.firstChild?.nextSibling
    if (!expr) {
  ...
  if (currNode.name === 'BinaryExpression') {
    ...
    const left = currNode.firstChild
    const op = left?.nextSibling
    const right = op?.nextSibling
    if (!left || !op || !right) {
    ...
    const expectedInputKind = op.name === 'CompareOp' ? iwInterface.VarKind.ANY : expectedOutputKind
  ...
  if (currNode.name === 'ConditionalExpression') {
    ...
    //   )
    // )
    const cond = currNode.firstChild
    const trueResult = cond?.nextSibling?.nextSibling
    const falseResult = trueResult?.nextSibling?.nextSibling
  ...
  const memberNameSplit = memberName.split('.')
  const namespace = memberNameSplit[1]
  const name = memberNameSplit[2]
  const ns = findMaterials(namespace)
  if (!ns) {
  ...
  if (kind === 'var') {
    const varInfo = (ns.items as iwInterface.VarInfo[]).find(item => item.name === name)
    if (!ns.isVar || !varInfo) {
    ...
    const funInfo = (ns.items as iwInterface.FunInfo[]).find(item => item.name === name)
    if (!funInfo) {
    ...
    const paramStartOffset = tempParamNode.nextSibling!.from - 1
    ...
    while (tempParamNode.nextSibling != null) {
      ...
      const paramName = tempParamNode.name
      ...
      const tempInputKind = funInfo.input[paramIdx].kind
